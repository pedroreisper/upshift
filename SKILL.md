---
name: upshift
description: Makes Claude Code match the execution gear to the shape of the work — and reach for it WITHOUT being told the feature's name. Classifies each request before acting and picks across the full capability surface: do it inline (small), fan out to parallel sub-agents (multi-domain), escalate to a Workflow / ultracode run (large, repetitive, parallelisable), push to a background agent (long-running independent job), or schedule it (polling / recurring). Use when the user says "be thorough", "do all of these", "migrate everything", "audit the whole codebase", "keep checking", "run this in the background", "faz tudo", "trata de tudo", "em massa", "exaustivo", "de forma autónoma", or whenever a task is clearly bigger than a single inline pass but the user did NOT name a feature. Also fires proactively (UserPromptSubmit hook) with a soft gear hint. NOT a keyword→skill router (it routes execution capabilities, not skills) and NOT a verifier (that's did-it-actually).
license: MIT
metadata:
  version: "1.0.0"
  priority: "8"
